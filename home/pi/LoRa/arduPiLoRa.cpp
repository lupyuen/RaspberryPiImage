	...
	{ // LoRa mode
		...
		else
		{
			...
			{ // CRC incorrect
				...
				#ifdef TP_IOT_bypass_crc  ////  TP-IoT
				////  Bypass CRC checking since Arduino doesn't send CRC.
				printf("** TP-IoT: Bypass CRC check **\n");
				state = 2;
				p_received = true;	// Assume packet correctly received
				_reception = CORRECT_PACKET;
				#endif //  TP_IOT_bypass_crc  ////  TP-IoT
			}
		}
		//writeRegister(REG_OP_MODE, LORA_STANDBY_MODE);	// Setting standby LoRa mode
	}
