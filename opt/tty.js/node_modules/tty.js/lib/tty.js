...
Server.prototype.initRoutes = function() {
  ...
  ////  Lup Yuen: Check the token on connection.
  this.get('/', function(req, res, next) {
    return self.checkToken(req, res, next);
  });
};

Server.prototype.checkToken = function(req, res, next) {
  ////  Lup Yuen: Check that we received a valid token from Ajenti: http://...?token=xxx
  var self = this
    , conf = this.conf;
  ...
  var errorResult = '<meta http-equiv="refresh" content="2; url=' + url + '">';

  res.contentType('.html');
  var token = req.query.token;
  console.log(token);
  if (!token) {
    console.error('Missing token', req);
    return res.send(errorResult + 'Missing token');
  }
  var filename = '/tmp/tty' + token;
  //  Delete the token file.
  fs.unlink(filename, function(err, data) {
    //  Unable to delete the token file.  Means it has already been used.
    if (err) {
      console.error(err);
      return res.send(errorResult + 'Invalid token');
    }
    try {
      //  Deleted the token file, so this request is OK.  Return the index.html.
      var index = __dirname + '/index.html';
      fs.readFile(index, 'utf8', function(err, data) {
        if (err) {
          console.error(err);
          return res.send(errorResult + err);
        }
        return res.send(data);
      });
    } catch(e) {
      console.error(e);
      return res.send(errorResult + e);
    }
  });
};
